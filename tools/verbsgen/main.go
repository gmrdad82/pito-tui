// Command verbsgen reads pito's grammar config (config/pito/verbs.yml in the
...
//
//	PITO_REPO=/path/to/pito go run ./tools/verbsgen
//
// PITO_REPO defaults to $HOME/Dev/pito. The grammar is read from that
// repo's COMMITTED tree (git show PITO_REF:config/pito/verbs.yml,
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, "verbsgen:", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	srcRel := filepath.Join("config", "pito", "verbs.yml")
	...
	// verbs.yml has two exact-duplicate consecutive lines (`introducer: without`
	...
	snap := snapshot{
		Source: source{
			...
			Note: "generated by tools/verbsgen — do not edit; re-run: go generate ./...",
		},
	...
	for _, kv := range mapPairs(mapGet(root, "verbs")) {
		...
		snap.Verbs = append(snap.Verbs, buildVerb(name, vNode))

		if segNode := mapGet(vNode, "segments"); segNode != nil {
			snap.Segments[name] = buildSegments(segNode)
		}
		// capabilities.{columns,filters} live only on the `list` verb.
	...
	fmt.Fprintf(os.Stderr, "verbsgen: wrote %s (%d verbs, %d vocabularies)\n", outPath, len(snap.Verbs), len(snap.Vocabularies))
...
// ---- verbs-inventory.md — the human-readable verb table (owner's item 4) ----

// verbStatus is one entry of the hand-kept overlay docs/claude/verb-status.yml.
type verbStatus struct {
	Status string `yaml:"status"`
	Note   string `yaml:"note"`
}

// writeInventory merges the grammar snapshot with the hand-kept status
// overlay into docs/claude/verbs-inventory.md. The overlay (and the doc)
...
func writeInventory(root string, snap snapshot) error {
	overlayPath := filepath.Join(root, "docs", "claude", "verb-status.yml")
	rawOverlay, err := os.ReadFile(overlayPath)
	if os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, "verbsgen: %s absent — skipping verbs-inventory.md\n", overlayPath)
		return nil
	}
	if err != nil {
		return fmt.Errorf("reading %s: %w", overlayPath, err)
	}
	overlay := map[string]verbStatus{}
	...
	b.WriteString("# PITO verb inventory\n\n")
	b.WriteString("<!-- generated by tools/verbsgen — do not edit by hand.\n")
	b.WriteString("     sources: pito config/pito/verbs.yml + docs/claude/verb-status.yml (hand-kept overlay).\n")
	b.WriteString("     regenerate: mise exec -- go generate ./... -->\n\n")
	fmt.Fprintf(&b, "%d verbs. Status legend: ok · needs-work · empty-data · excluded · server-missing · unvisited.\n\n", len(snap.Verbs))
	b.WriteString("| Verb | Aliases | Auth | Surfaces | Reply verbs | Status | Note |\n")
	b.WriteString("|---|---|---|---|---|---|---|\n")
	...
	for _, v := range snap.Verbs {
		...
		if !ok {
			st = verbStatus{Status: "unvisited"}
		}
	...
	if len(snap.UniversalReply) > 0 {
		...
		b.WriteString("Offered on every followupable message (not verbs; `Share::UniversalActions`):\n\n")
	...
	if len(orphans) > 0 {
		...
		b.WriteString("\n## Overlay entries with no matching verb\n\n")
...
func buildVerb(name string, n *yaml.Node) verb {
	v := verb{
		Name:           name,
	...
	// auth precedence: top-level `auth:` (chat verbs) > slash.auth (slash-only
	// verbs, e.g. login/config/jobs) > chat.auth (unused today, kept as a
	// fallback in case a future verb nests it there).
	switch {
